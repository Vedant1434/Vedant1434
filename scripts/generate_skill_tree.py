...
class AdvancedProfileAnalyzer:
    ...
    def _analyze_repo(self, repo: Dict, contrib_stats: Dict):
        ...
        if pushed_at:
            try:
                ...
                recency = max(0.3, 1.0 - (days_old / 730))
            except: pass
    ...
    def _detect_frameworks(self, lang: str, text: str):
        ...
        for framework, keywords in tech['frameworks'].items():
            if any(kw in text for kw in keywords):
                self.skills[lang]['frameworks'][framework] += 10
    ...
    def _process_skills(self, contrib_stats: Dict) -> List[Dict]:
        ...
        for lang, data in self.skills.items():
            ...
            top_frameworks = sorted(data['frameworks'].items(), key=lambda x: x[1], reverse=True)[:5]
